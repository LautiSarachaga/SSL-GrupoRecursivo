...
class Lexer:
    def __init__(self, codigo):
        ...
        self.linea = 1
        self.columna = 1
        self.errores = [] # Lista para almacenar errores léxicos sin detener la ejecución
    ...
    def leer_numero_con_unidad(self):
        ...
        raise ValueError(
            f"Error Léxico en línea {self.linea}: El número '{num}' no tiene unidad válida."
        )
    ...
    def leer_string(self):
        ...
        # Soportamos tanto comillas estándar como comillas tipográficas (comunes en copy/paste)
        ...
        raise ValueError(f"Error Léxico en línea {self.linea}: Cadena sin cerrar.")
    ...
    def leer_email(self):
        ...
        if valor.count("@") != 1:
            raise ValueError(f"Error Léxico en línea {self.linea}: Email inválido '{valor}'.")
        ...
        for char in usuario + dominio:
            if not (char.isalnum() or char in caracteres_permitidos):
                raise ValueError(f"Error Léxico en línea {self.linea}: Carácter no permitido en email '{char}'.")

        extension = dominio.split(".")[-1]
        if not (2 <= len(extension) <= 4) or not extension.isalpha():
            raise ValueError(f"Error Léxico en línea {self.linea}: Extensión de email inválida '{extension}'.")
    ...
    def tokenizar(self):
        ...
        while self.pos < len(self.codigo):
            ...
            try:
                # Comillas dobles estándar y tipográficas
                if char in ['"', '”', '“']:
                    ...
                    i = self.pos
                    es_email = False
                    while i < len(self.codigo) and not self.codigo[i].isspace() and self.codigo[i] not in ['\n', '\r']:
                        if self.codigo[i] == "@":
                            es_email = True
                            break
                        i += 1
                    ...
                    if token:
                        ...
                        raise ValueError(f"Error Léxico en línea {self.linea}: Operador '{char}' inválido.")

                else:
                    raise ValueError(f"Error Léxico en línea {self.linea}: Símbolo '{char}' no soportado.")
                    
            except ValueError as e:
                self.errores.append(str(e))
                # Avanzamos un caracter para no entrar en bucle infinito
                self.pos += 1
                self.columna += 1
